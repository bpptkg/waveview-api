...
class DetectionResult:
    ...
    def get_sof_pick(self) -> PickResult | None:
        SOF_STATIONS = ["MEPAC", "MEPSL"]
        for station in SOF_STATIONS:
            for pick in self.picks:
                station_code = pick.stream_id.split(".")[1]
                if station_code == station and pick.t_pick is not None:
                    return pick
        return None
        ...
        if len(st_ref) > 0:
            ...
            if len(trace1_raw.data) > 0:
                ...
                tr1 = (
                    ...
                    .detrend()
                    .taper(0.01)
                    .filter("bandpass", freqmin=3, freqmax=15)
                )
                tr2 = (
                    trace.copy()
                    .detrend()
    ...
    def _process_event(self, duration: float):
        ...
        for tr in self.stream_buffer:
            ...
            if len(trimmed.data) >= 1500:
                ...
                t_pick = pick_res[3]
                self._log(
                    f"PICK for {tr.stats.station}: {t_pick} ({t_pick - self.t_on:.2f}s after onset)"
                )
                picks.append(
                    PickResult(
                        stream_id=tr.id,
        ...
        event_result = DetectionResult(t_on=self.t_on, t_off=self.t_off, picks=picks)
